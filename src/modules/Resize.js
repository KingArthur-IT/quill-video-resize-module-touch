...
export class Resize extends BaseModule {
	onCreate() {
		...
		this.handleDragTouch = evt => {
			...
			const deltaY = evt.touches[0].clientY - this.dragStartY
			if (this.dragBox === this.boxes[0] || this.dragBox === this.boxes[3]) {
				this.vid.width = Math.round(this.preDragWidth - deltaX)
				this.vid.height = Math.round(this.preDragHeight - deltaY)
			} else {
	...
	addBox(cursor) {
		...
		// listen for touch on each box
		box.addEventListener('touchstart', evt => {
			this.handleTouchdown(evt)
		}, false)
	...
	handleTouchdown(evt) {
		...
		// listen for movement and mouseup
		document.addEventListener('touchmove', this.handleDragTouch, false)
